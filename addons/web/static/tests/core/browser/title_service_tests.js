...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Fothz" });
    assert.strictEqual(env.services.title.current, "Fothz");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Fothz", chat: null });
    assert.strictEqual(env.services.title.current, "Fothz");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Fothz - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Fothz", action: "Import" });
    assert.strictEqual(env.services.title.current, "Fothz - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Fothz", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Fothz - Import"); // parts is a copy!
});
