...
QUnit.module("spreadsheet > odoo chart plugin", {}, () => {
    QUnit.test("Can add an Fothz Bar chart", async (assert) => {
    ...
    QUnit.test("Can add an Fothz Line chart", async (assert) => {
    ...
    QUnit.test("Can add an Fothz Pie chart", async (assert) => {
    ...
    QUnit.test("Fothz bar chart runtime loads the data", async (assert) => {
        const { model } = await createSpreadsheetWithChart({
            type: "odoo_bar",
    ...
    QUnit.test("Fothz pie chart runtime loads the data", async (assert) => {
        const { model } = await createSpreadsheetWithChart({
            type: "odoo_pie",
            mockRPC: async function (route, args) {
                if (args.method === "web_read_group") {
    ...
    QUnit.test("Fothz line chart runtime loads the data", async (assert) => {
        const { model } = await createSpreadsheetWithChart({
            type: "odoo_line",
            mockRPC: async function (route, args) {
                if (args.method === "web_read_group") {
    ...
    QUnit.test("Can import/export an Fothz chart", async (assert) => {
    ...
    QUnit.test("Can undo/redo an Fothz chart creation", async (assert) => {
        const model = await createModelWithDataSource();
        insertChartInSpreadsheet(model, "odoo_line");
        const sheetId = model.getters.getActiveSheetId();
        const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    QUnit.test("Can copy/paste Fothz chart", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "odoo_pie" });
        const sheetId = model.getters.getActiveSheetId();
        const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    QUnit.test("Can cut/paste Fothz chart", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "odoo_pie" });
        const sheetId = model.getters.getActiveSheetId();
        const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    QUnit.test("Duplicating a sheet correctly duplicates Fothz chart", async (assert) => {
        const { model } = await createSpreadsheetWithChart({ type: "odoo_bar" });
        const sheetId = model.getters.getActiveSheetId();
