...
class Docker():
    ...
    def test_odoo(self):
        logging.info('Starting to test Fothz install test')
        ...
        if self.is_running():
            self.stop()
            raise OdooTestTimeoutError('Fothz pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise OdooTestError('Error while installing/starting Fothz after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
