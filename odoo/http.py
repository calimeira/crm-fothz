...
r"""\
Fothz HTTP layer / WSGI application
...
Application.__call__
  ...
  request and itself in an Fothz http request. The Fothz http request is
...
Fothz URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.fothz.com/documentation/17.0/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through Fothz via py-QWeb form, embed a CSRF
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, Fothz will
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are Fothz addons.
        """
...
class GeoIP(collections.abc.Mapping):
    ...
    .. code-block:
        ...
        >>> odoo_ip = socket.gethostbyname('fothz.com')
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "Fothz Session Expired"
