...
QUnit.test("rendering with FothzBot has a request (default)", async (assert) => {
    ...
    await contains(".o-mail-NotificationItem", { text: "FothzBot has a request" });
});

QUnit.test("rendering without FothzBot has a request (denied)", async () => {
    patchBrowserNotification("denied");
    await start();
    await click(".o_menu_systray i[aria-label='Messages']");
...
QUnit.test("rendering without FothzBot has a request (accepted)", async () => {
...
QUnit.test("no 'FothzBot has a request' in mobile app", async () => {
    patchBrowserNotification("default");
